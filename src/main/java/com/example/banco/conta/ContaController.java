...
public class ContaController {
    ...
    @PutMapping("/sacar/{id}")
    @PreAuthorize("hasAnyRole(T(com.example.banco.usuario.Role).ADMIN.name(),T(com.example.banco.usuario.Role).FUNCIONARIO.name(),T(com.example.banco.usuario.Role).CLIENTE.name())")
    ...
    @PutMapping("/investir/{id}")
    @PreAuthorize("hasAnyRole(T(com.example.banco.usuario.Role).ADMIN.name(),T(com.example.banco.usuario.Role).FUNCIONARIO.name(),T(com.example.banco.usuario.Role).CLIENTE.name())")
    public ResponseEntity<ContaDTO> investir(@PathVariable("id") Long id, @RequestBody ContaDTO dto) {
        dto.setId(id);
        return new ResponseEntity<>(this.service.investir(dto), HttpStatus.OK);
    }
